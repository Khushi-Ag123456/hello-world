...
placeOrder(orderModel: OrderDetails) {
  ...
  alert( this.OrdersToString(orderModel));
  localStorage.setItem(orderModel.OrderDate.toString(), this.OrdersToString(orderModel));
}
...
stringToOrders(str) {
  ...
  var itemArray : Item[]= [];
  var tempStr = str.split("<");
  var obj = JSON.parse(tempStr[5]);
  console.log(typeof(obj));
  // var stringArr = tempStr[5].split(",");
  // for(var i = 0; i < stringArr.length; i++) {
  //   //  var obj = JSON.parse(JSON.stringify(stringArr[i]));
  // }
      this.order = new OrderDetails(String(tempStr[0]), String(tempStr[1]),
          String(tempStr[2]), String(tempStr[3]), tempStr[4], [tempStr[5]]);
      // this.order = new OrderDetails(str.Name.toString(), str.Address1.toString(), str.Address2.toString(), Date.parse(str.City.toString()), str.OrderDate, str.OrderItems);
      return this.order;
}
